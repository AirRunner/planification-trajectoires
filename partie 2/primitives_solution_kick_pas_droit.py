...
class KickBall(Action):
    def __init__(self, moveGroup, ball, room):
        Action.__init__(self, moveGroup)
        self.ball = ball
        #self.dir = direction 
        obj = scene.get_objects([self.ball.name])
    ...
    def execute(self):
        ...
        # Compute the angle between the robot and the ball. We want the robot to face the ball.
        angle = -math.atan2(self.obj_y - self.destRoom.y_coord, self.obj_x - self.destRoom.x_coord)
        ...
        # Compute the distance between the robot and the edge of the ball.
        distance = math.sqrt((self.obj_x - base_x) ** 2 + (self.obj_y - base_y) ** 2) 
        
        # + PR2_RADIUS + BALL_RADIUS
        
        #- (BALL_RADIUS + PR2_RADIUS)
        ...
        print("Position de la balle \n  x : {0} | y : {1} | z : {2} ".format(round(self.obj_x,1),self.obj_y,42))
        ...
        if isPlanValid(plan):
            self.moveGroup.execute(plan)
            # Ici on est a cote de la balle mais mal alligne
            ...
            ballPose.pose.position.x = self.obj_x + 2*math.cos(angle)
            ballPose.pose.position.y = self.obj_y + 2*math.sin(angle)
    ...
    def __isPreconditionValid(self):
        ...
        pos_x = self.obj_x + 2*math.cos(angle+math.pi)
        pos_y = self.obj_y + 2*math.sin(angle+math.pi)
...
if __name__ == '__main__':
    try:
        ...
        # On choisit de ne shooter que pour la salle 5 car on economise ainsi deux action (move,kick)
        # Pour les salles 1 a 4, on fait un deplacement 'classique' (move,pick,move,drop)
        # C'est aussi efficace que de faire avec 2 kick, mais plus simple a implementer 


        actions.append(Move(moveGroup, room1))
        actions.append(KickBall(moveGroup,ball1,room1centre))

        actions.append(Move(moveGroup, room1centre))
        actions.append(KickBall(moveGroup,ball1,room0))
        ...
        actions.append(KickBall(moveGroup,ball5,room0))
